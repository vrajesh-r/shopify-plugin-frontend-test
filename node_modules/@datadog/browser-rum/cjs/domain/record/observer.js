...
function initObservers(o) {
    var mutationHandler = initMutationObserver(o.mutationController, o.mutationCb);
    ...
    var mediaInteractionHandler = initMediaInteractionObserver(o.mediaInteractionCb);
...
function initMutationObserver(mutationController, cb) {
    return mutationObserver_1.startMutationObserver(mutationController, cb).stop;
}
...
function initMouseInteractionObserver(cb) {
    var handler = function (event) {
        var target = event.target;
        if (privacy_1.nodeOrAncestorsShouldBeHidden(target) || !serializationUtils_1.hasSerializedNode(target)) {
            return;
        }
        var _a = utils_1.isTouchEvent(event) ? event.changedTouches[0] : event, clientX = _a.clientX, clientY = _a.clientY;
        cb({
...
function initScrollObserver(cb) {
    var updatePosition = browser_core_1.throttle(browser_core_1.monitor(function (event) {
        var target = event.target;
        if (!target || privacy_1.nodeOrAncestorsShouldBeHidden(target) || !serializationUtils_1.hasSerializedNode(target)) {
            return;
        }
...
function initInputObserver(cb) {
    function eventHandler(event) {
        var target = event.target;
        if (!target || !target.tagName || !browser_core_1.includes(exports.INPUT_TAGS, target.tagName) || privacy_1.nodeOrAncestorsShouldBeHidden(target)) {
            return;
        }
        ...
        if (type === 'radio' || type === 'checkbox') {
            ...
            var value = serializationUtils_1.getElementInputValue(target);
            if (value === undefined) {
                return;
            }
...
function initMediaInteractionObserver(mediaInteractionCb) {
    var handler = function (event) {
        var target = event.target;
        if (!target || privacy_1.nodeOrAncestorsShouldBeHidden(target) || !serializationUtils_1.hasSerializedNode(target)) {
            return;
