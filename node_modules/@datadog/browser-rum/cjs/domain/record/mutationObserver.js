 ...
 */
function startMutationObserver(controller, mutationCallback) {
...
function processMutations(mutations, mutationCallback) {
    ...
    var filteredMutations = mutations.filter(function (mutation) {
        return document.contains(mutation.target) &&
            ...
            !privacy_1.nodeOrAncestorsShouldBeHidden(mutation.target);
    });
    var _a = processChildListMutations(filteredMutations.filter(function (mutation) { return mutation.type === 'childList'; })), adds = _a.adds, removes = _a.removes, hasBeenSerialized = _a.hasBeenSerialized;
...
function processChildListMutations(mutations) {
    ...
    for (var _a = 0, sortedAddedAndMovedNodes_1 = sortedAddedAndMovedNodes; _a < sortedAddedAndMovedNodes_1.length; _a++) {
        ...
        var serializedNode = serialize_1.serializeNodeWithId(node, {
            ...
            ancestorInputPrivacyMode: privacy_1.getNodeOrAncestorsInputPrivacyMode(node.parentNode),
        });
        if (!serializedNode) {
            continue;
        }
...
function processCharacterDataMutations(mutations) {
    var textMutations = [];
...
function processAttributesMutations(mutations) {
    ...
    for (var _i = 0, filteredMutations_2 = filteredMutations; _i < filteredMutations_2.length; _i++) {
        ...
        if (mutation.attributeName === 'value') {
            var inputValue = serializationUtils_1.getElementInputValue(mutation.target);
            ...
            transformedValue = serializationUtils_1.transformAttribute(document, mutation.attributeName, value);
        }
        else {
